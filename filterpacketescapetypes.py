...
def main() -> None:
    ...
    for filein in Path().glob("packets*.out.*"):
        ...
        if any(get_type_escapetype(line) != (TYPE_ESCAPE, TYPE_RPKT) for line in linesin if not line.startswith("#")):
            ...
            print(f"  backing up {filein} to {backupfolder / filein.name}...")
            filein.rename(backupfolder / filein.name)
            fileout_rpkt = Path(*filein.parts[:-1], filein.parts[-1].removesuffix(".zst").removesuffix(".gz").removesuffix(".xz") + ".zst")
            fileout_rpkt_temp = Path(*filein.parts[:-1], filein.parts[-1] + ".partialtmp")
            print(f"  writing filtered rpkts to {fileout_rpkt}...", end="")
            kept_packets = 0
            ...
            with compression.zstd.open(fileout_rpkt_temp, "wt", level=12) as foutrpkt:
                for line in linesin:
                    ...
                    if type_id == str(at.packets.type_ids["TYPE_ESCAPE"]) and escape_type_id == str(at.packets.type_ids["TYPE_RPKT"]):
                        foutrpkt.write(line)
                        kept_packets += 1
            size_factor =  fileout_rpkt_temp.stat().st_size /(backupfolder / filein.name).stat().st_size
            print(f" kept {kept_packets} of {len(linesin)} packets ({kept_packets / len(linesin) * 100:.2f}%) new/old size {size_factor*100:.2f}%")
